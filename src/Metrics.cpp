...
//  hasUnitWeights
//  Verifica si todas las aristas tienen peso 1.0 para usar BFS en vez de Dijkstra.
...
//  Construye conjuntos de vecinos para calcular clustering local sin repetir nodos.
std::vector<std::unordered_set<int>> construirConjuntosDeVecinos(const Graph &graph) {
	const int vertexCount = graph.getNumVertices();
	std::vector<std::unordered_set<int>> neighbors(vertexCount);

	// Carga vecinos salientes y, si el grafo es dirigido, también los entrantes.
...
//  calcularDistanciasCaminosMinimos
//  Elige BFS o Dijkstra según los pesos del grafo y valida pesos negativos.
std::vector<double> calcularDistanciasCaminosMinimos(const Graph &graph, int source) {
		// Rechaza pesos negativos porque estas métricas asumen distancias no negativas.
	if (hasNegativeWeight(graph)) {
		throw std::invalid_argument(
			"Metrics: Las metricas de shortest-path requieren pesos no negativos");
	}
		// Si todas las aristas pesan 1.0, BFS es la opción más simple.
	if (hasUnitWeights(graph)) {
		return calcularDistanciasBFS(graph, source);
	}
		// En cualquier otro caso usa Dijkstra.
	return calcularDistanciasDijkstra(graph, source);
}
...
std::vector<double> degreeCentrality(const Graph &graph, DegreeMode mode) {
	...
	if (mode == DegreeMode::Out) {
		for (int vertex = 0; vertex < vertexCount; ++vertex) {
			centrality[vertex] = static_cast<double>(graph.degree(vertex)) * normalization;
		}
		return centrality;
	}
	...
	if (mode == DegreeMode::Total && !graph.getIsDirected()) {
		...
		// Para In y Total en grafos dirigidos, reconstruye el in-degree recorriendo las aristas.
	...
	for (int vertex = 0; vertex < vertexCount; ++vertex) {
		if (mode == DegreeMode::In) {
			centrality[vertex] = static_cast<double>(inDegree[vertex]) * normalization;
		} else {
			centrality[vertex] = static_cast<double>(graph.degree(vertex) + inDegree[vertex]) * normalization;
		}
	}
	...
	for (int source = 0; source < vertexCount; ++source) {
		...
		while (!queue.empty()) {
			...
			for (const Edge &edge : graph.getNeighbors(vertex)) {
				...
				} else if (std::fabs(candidate - distance[edge.destination]) <= kEpsilon) {
					sigma[edge.destination] += sigma[vertex];
					predecessors[edge.destination].push_back(vertex);
				}
			}
		...
		while (!stack.empty()) {
			...
			stack.pop_back();
				...
				// Distribuye la dependencia entre los predecesores del vértice actual.
			for (int predecessor : predecessors[vertex]) {
				if (sigma[vertex] > 0.0) {
					dependency[predecessor] +=
						(sigma[predecessor] / sigma[vertex]) * (1.0 + dependency[vertex]);
				}
			}
...
//  closenessCentrality
//  Calcula la centralidad de cercanía a partir de caminos mínimos desde cada nodo.
std::vector<double> closenessCentrality(const Graph &graph) {
	...
	// Calcula la cercanía de cada vértice sumando sus distancias hacia los demás.
	for (int source = 0; source < vertexCount; ++source) {
		const std::vector<double> distancias = calcularDistanciasCaminosMinimos(graph, source);
		double reachable = 0.0;
		double sum = 0.0;

		for (int target = 0; target < vertexCount; ++target) {
			// Ignora la distancia al propio nodo y solo acumula distancias finitas.
			if (target == source) {
				continue;
			}
...
//  pageRank
//  Calcula PageRank por iteración hasta converger o alcanzar el máximo de iteraciones.
std::vector<double> pageRank(const Graph &graph, double damping,
							 int maxIterations, double tolerance) {
	if (damping < 0.0 || damping > 1.0) {
		throw std::invalid_argument("Metrics::pageRank: damping debe estar entre [0, 1]");
	}
	if (maxIterations <= 0) {
		throw std::invalid_argument("Metrics::pageRank: maxIterations debe ser positivo");
	}
	...
	// La parte base evita que el rango desaparezca por completo en nodos aislados.
	const double baseRank = (1.0 - damping) / static_cast<double>(vertexCount);

	// Calcula el out-degree de cada vértice para repartir el rango en cada iteración.
	...
	for (int iteration = 0; iteration < maxIterations; ++iteration) {
		...
		for (int vertex = 0; vertex < vertexCount; ++vertex) {
			// Los vértices sin salidas acumulan masa que luego se reparte uniformemente.
			...
			const double share = damping * rank[vertex] / static_cast<double>(outDegree[vertex]);
			for (const Edge &edge : graph.getNeighbors(vertex)) {
		...
		const double danglingShare = damping * danglingMass / static_cast<double>(vertexCount);
...
std::vector<double> localClusteringCoefficient(const Graph &graph) {
	...
	const std::vector<std::unordered_set<int>> vecinos = construirConjuntosDeVecinos(graph);
	...
	for (int vertex = 0; vertex < vertexCount; ++vertex) {
		...
		// Recorre cada par de vecinos y cuenta los que sí están conectados entre sí.
		std::vector<int> lista(vecinosAdyacentes.begin(), vecinosAdyacentes.end());
		int links = 0;
		...
		for (std::size_t i = 0; i < lista.size(); ++i) {
			for (std::size_t j = i + 1; j < lista.size(); ++j) {
				...
				// Se considera conectado si aparece en cualquiera de las dos direcciones.
		...
		coefficient[vertex] = (2.0 * static_cast<double>(links)) /
